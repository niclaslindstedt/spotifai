...
//!
//! Both commands follow the same shape: ensure the pinned zad binary is
//! on disk, scaffold the matching `~/.spotifai/permissions/<profile>.toml`
//! if missing, render a Markdown prompt template with the policy injected,
//! set [`crate::api::SPOTIFAI_PROFILE_ENV`] so the child `spotifai api`
//! shells route to the correct policy file, and then hand control over
//! to zag.
//!
//! The prompt template format is shared too — see
//! [`prompts/README.md`](../prompts/README.md). The `## System` section
//! is what gets sent to the model; the `## User` block is for the
//! authoring workflow only and is dropped here.

use anyhow::{Context, Result};
use zag::builder::AgentBuilder;

use crate::api::SPOTIFAI_PROFILE_ENV;
use crate::install;
use crate::output;
...
///
/// `command_label` is the display name printed to stderr when the session
/// starts (e.g. `"ask"`, `"playlist"`). `prompt_template` is the raw
/// Markdown file as baked in via `include_str!`. `initial_prompt` becomes
/// the agent's first turn; `None` opens the session empty.
pub fn run_agent(
    profile: Profile,
    command_label: &str,
    prompt_template: &str,
    ...
    // first run creates a default; subsequent runs are a no-op so user
    // edits are preserved.
    let (policy_path, wrote) = permissions::ensure_default_for(profile)?;
    if wrote {
    ...
    let policy = permissions::read_or(&policy_path, profile.default_policy())?;

    let system_prompt = render_system_prompt(prompt_template, &policy);

    // Pin the active profile so child `spotifai api` shells resolve to
    // the same file we just rendered into the prompt. Set on this
    // process's env so it propagates to anything zag spawns. The
    // spotifai process handles exactly one command per invocation, so
    // a global set is safe.
    //
    ...
    output::header(&format!("spotifai {command_label}"));
