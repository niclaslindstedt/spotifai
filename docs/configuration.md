...
| `spotify` × `clean`     | `~/.spotifai/permissions/spotify/clean.toml`    | Read + destructive — `playlists delete`, `playlists remove`, `library tracks unsave`, `library albums unsave`. `search` and every creator verb (`playlists create|add|rename`, library saves) stay denied. |
...
| `ymusic` × `clean`      | `~/.spotifai/permissions/ymusic/clean.toml`     | Read + destructive — `playlists delete`, `playlists remove`, `library unlike`. `search` and every creator verb (`playlists create|add|rename`, `library like`) stay denied. |

All files share the same TOML schema:

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `mode` | string | `read_only` (ask) / `playlist_curator` (playlist) / `library_cleanup` (clean) | Free-form tag for the policy. Informational — the effective gate is the `allowed` / `denied` lists below. |
...
All six files are created with their defaults the first time `spotifai install` runs; subsequent runs leave any hand edits in place. To widen or narrow any profile, edit `allowed` / `denied` directly, then re-run `spotifai install` so the file is resigned and zad's load-time trust check accepts it. spotifai re-reads the matching file on every `spotifai ask` / `spotifai playlist` / `spotifai clean` / `spotifai export` invocation. To rewrite a file back to its default, delete it and re-run `spotifai install`.
...
|---|---|
| `SPOTIFAI_PROVIDER` | Internal coupling between `spotifai ask` / `playlist` / `clean` / `export` and `spotifai api`. The parent command sets this to `spotify`, `ymusic`, … before launching zag. Unset is treated as `spotify` for backwards compatibility; an unknown value fails with a usage error. |
| `SPOTIFAI_PROFILE`  | Internal coupling between `spotifai ask` / `playlist` / `clean` and `spotifai api`. The parent command sets this to `ask`, `playlist`, or `clean` before launching zag, and `spotifai api` reads it to pick which file to point zad at. **Not a user knob** for the profile axis: direct `spotifai api …` invocations from a shell error out with a usage message. To call zad outside spotifai, run `~/.spotifai/bin/zad <provider> …` with `ZAD_PERMISSIONS_PATH` set yourself. |
...
|------|------|---------|-------------|
| `--yolo` | flag | off | Forward `auto_approve(true)` to the underlying zag `AgentBuilder` so the interactive surfaces (`spotifai ask`, `spotifai playlist`, `spotifai clean`) skip every per-tool approval prompt. The `(provider, profile)` policy file is still enforced by `spotifai api` at the zad layer, so `--yolo` cannot widen the allowed verb list — it only suppresses zag's tool-approval gating on top. The `clean` profile additionally bakes a confirmation step into the prompt, which `--yolo` does not remove. No-op for the one-shot commands (`api`, `auth`, `export`, `import`, `install`) since they do not spawn zag. Global — works on every subcommand. |
