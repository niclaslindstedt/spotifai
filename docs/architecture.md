...
Spotify and YouTube Music both enforce rolling-window rate limits per application, and the practical consequence of hammering them through a rate-limit response is a longer cooldown that affects every concurrent caller — including unrelated sibling processes that share the OAuth client. zad keeps a per-service deadline file at `~/.zad/state/<service>/rate_limit.json`: on any 429 response the service client parses `Retry-After`, writes the absolute deadline, and returns `ZadError::RateLimited`. A classifier in `zad::google_quota` promotes Google's quota-flavoured `HTTP 403` bodies (`quotaExceeded`, `dailyLimitExceeded`, `rateLimitExceeded`, `userRateLimitExceeded`) into the same `ZadError::RateLimited` shape; YouTube Music's runtime client talks to the internal **InnerTube** backend at `music.youtube.com/youtubei/v1`, which is not metered by the Data API daily quota and rarely trips that path in practice, but the classifier is kept as cheap insurance. Spotifai calls `zad::rate_limit::precall_check(service, wait)` before every zad operation (`src/api.rs`, `src/export.rs`, `src/import.rs`); with `wait = true` the helper sleeps until the deadline (capped at one hour per invocation — daily-quota deadlines that exceed the cap will sleep an hour and then surface `RateLimited` so the user can decide whether to keep waiting), with `wait = false` it returns the same error so the caller fails fast without burning a request.
...
6. zad executes the API call against the active provider (Spotify Web API for Spotify, the InnerTube backend at `music.youtube.com/youtubei/v1` for YouTube Music, …) and returns the typed response; `spotifai api` serialises it to JSON and writes it to stdout. zag synthesises a natural-language response from the tool output.
