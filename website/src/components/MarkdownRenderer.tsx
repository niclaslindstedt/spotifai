...
function makeComponents(basePath: string): Components {
  return {
    ...
    h6: heading("h6"),
    a({ href, children }) {
      // Markdown corpora cross-link via `<file>.md` (or relative
      // paths like `../docs/<file>.md`). Rewrite each into the
      // matching internal route under `basePath`.
      if (href && /\.md$/.test(href)) {
        const stem = href.replace(/^.*\//, "").replace(/\.md$/, "");
        const target = stem === "main" && basePath === "/manual" ? "main" : stem;
        return (
          <Link
            to={`${basePath}/${target}`}
