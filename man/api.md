...
| `SPOTIFAI_WAIT` | read | Controls how `spotifai api` reacts when zad's shared rate-limit deadline file (`~/.zad/state/<service>/rate_limit.json`) shows the active provider is still in a cooldown window. Spotify writes the deadline on `HTTP 429`; YouTube Music writes it on `HTTP 429` *or* on `HTTP 403` with a Google quota body (`quotaExceeded`, `dailyLimitExceeded`, `rateLimitExceeded`, `userRateLimitExceeded`) — zad 0.9.0 funnels both shapes through `ZadError::RateLimited`. `1`/`true`/`yes`/`on` → sleep until the deadline and continue (capped at one hour per invocation — ymusic daily quotas that exceed the cap will sleep an hour and then re-surface the error); `0`/`false`/`no`/`off` → return an error wrapping `zad::ZadError::RateLimited` so the caller fails fast. Set on the user's behalf by `spotifai ask` / `spotifai playlist` so sub-agent fan-outs coordinate on one quota. The CLI `--wait` / `--no-wait` flags override the env var. |
